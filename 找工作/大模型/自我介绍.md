...
面试官老师，您好！我的名字叫范峻铨，我是中科大计算机科学与技术专业的硕士研究生，2025年6月毕业。本科就读于湖南大学大学智能科学与技术专业，在本科期间积极参加各类比赛，曾获高教杯数学建模全国一等奖，推免至科大研究生。我在实验室的研究方向是基于大模型的知识库问答系统，研究生期间积极参加实验室项目和各类比赛，其中包括与百度合作的ESG领域的大模型检索问答系统，在领域文档中提取知识存入向量数据库，检索以及利用生成大模型回答。并且在相关比赛全球智能汽车AI挑战赛中获得较好的名次，除此之外，我还对端设备模型性能，推理优化有所研究。曾参加实验室与华为合作的端边协同推理优化项目，负责端设备模型性能评估工作，并且发表了一篇论CCF C文。我个人情况大概是这样，请问老师有什么问题吗。

df -h .

"Good morning，my name is fanjunquan I'm a graduate student in Computer Science and Technology at USTC, graduating in  2025. I completed my undergraduate studies at Hunan University. During my undergraduate studies, I actively join in various competitions and won a first-place prize in the Mathematical Modeling Contest. My graduate research focuses on knowledge-based question answering systems using large language models. I've been actively join in lab projects and competitions, including a project with Baidu on large language models retrieval question answering systems in the ESG domain, do much work including  indexing、retrieval and generation . I also got good rankings in the Global Intelligent Vehicle AI Challenge. Additionally, I have researched model performance and inference optimization on edge devices, join in a project on edge-cloud optimization with Huawei,  I  do some work for evaluating model performance on edge devices, and finish one paper. That's a bit about me. Do you have any questions?"




In project with Baidu, we are committed to developing a question-answering system focusing on ESG (Environmental, Social, Governance) knowledge in the finance domain.  we provide an extral database to aLarge Language Model , which retrieves relevant information from the database using information retrieval techniques base on queries. The LLM then integrates this information to generate results for the user. The main workflow include three steps:

1. index ：Chunking and storing text into a knowledge database.
2. retrieve：Retrieving relevant text based on user queries.
3. generate：The large model summarizing and providing answers based on the retrieved content.



Tesla is in a leading position internationally in the electric car domain.

I think it is exllent companies
