...
在一个全新的边缘GPU上预测一组CNN的性能而不在新的HW上训练性能预测模型的能力是无法实现的



由于这些嵌入式硬件平台最近在市场上被提出，其硬件配置数量有限。在没有额外训练或基准测试的情况下，新的未知HW的泛化是困难的。模型在新的设备上泛化困难



边缘GPU平台由于其复杂的集成性质而建模复杂，其中GPU和CPU共享相同的内存，并且它们的微处理器体系结构存在差异。



为这些边缘GPU构建精确的分析模型需要对CNN执行堆栈的整体和层次进行建模。该栈由CNN结构、深度学习SDK、编译器和硬件微架构组成。由于GPU厂商(例如NVIDIA CUDA编译器)对HW和SW的一些细节保持谨慎，这种方法非常具有挑战性。
>>>>>>> Stashed changes
...
​	在本文中我们首次去预测模型在真实民用移动手机上的时延，以及部署的测量困难（介绍arm大小核的调度）
...
（nn-meter是在移动cpu，移动GPU和intel VPU）
